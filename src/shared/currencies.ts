/**
 * The ISO-4217 register: the currencies this platform can settle in.
 *
 * A three-letter shape check is not a currency check: `zzz` matches `[a-z]{3}`
 * and would otherwise be accepted onto a price, a payment intent or a credit
 * grant, after which the pricing page happily renders "ZZZ 0.50" for the rest
 * of the price's life — and a price's currency can never be edited once
 * anything has billed against it. So the code is checked against the register
 * on the way in, where the mistake is still cheap to fix.
 *
 * It lives in the kernel next to `validate.ts` because `v.currency()` is the
 * door every module actually uses; catalog, payments and revenue each grew a
 * private wrapper around it when the check lived one module along, and three
 * copies of one list is how two of them drift.
 *
